"use client";
import {
    Button,
    Col,
    Form,
    message,
    Popconfirm,
    Row,
    Select,
    Space,
    Table,
    TablePaginationConfig, Tag,
} from "antd";
import {useRouter} from "next/navigation";
import {useEffect, useState} from "react";
import styles from "./index.module.css"
import dayjs from "dayjs";
import {getBookList} from "@/api/book";
import {borrowDelete} from "@/api/borrow";
import {getBorrowList} from "@/api/borrow";
import {BookType, BorrowQueryType, BorrowType, UserType} from "@/types/page";
import Content from "@/components/Content/page";
import {getUserList} from "@/api/user";

//借出状态搜索框设置
const STATUS_OPTIONS = [
    {
        label:"借出",
        value:"on",
    },
    {
        label:"归还",
        value:"off",
    },
]

// 表格标题
const COLUMNS = [
  {
    title: '名称',
    dataIndex: 'book',
    key: 'book',
    width: 200,
    render: (book: BookType) => book?.name ?? '-',
  },
  {
    title: '状态',
    dataIndex: 'status',
    key: 'status',
    width: 100,
    render: (text: string) => {
        return text === "on" ? <Tag color="red">借出</Tag> : <Tag color="green">已还</Tag>;
    },
  },
  {
    title: '借阅人',
    dataIndex: 'borrowUser',
    key: 'borrowUser',
    width: 100,
    render: (borrowUser: UserType) => borrowUser?.name ?? '-',
  },
  {
    title: '借阅时间',
    dataIndex: 'borrowAt',
    key: 'borrowAt',
      render: (text: string) => dayjs(text).format('YYYY-MM-DD')
  },
  {
    title: '归还时间',
    dataIndex: 'backAt',
    key: 'backAt',
      render: (text: string) => dayjs(text).format('YYYY-MM-DD')
  },
];


export default function Borrow() {
    const [form] = Form.useForm()  //有中括号是因为要对返回结果进行解构赋值
    const router=useRouter()
    // 分页变量值定义
    const [pagination, setPagination] = useState<TablePaginationConfig>({
        current:1,
        pageSize:20,
        showSizeChanger:true,
        total:0
    })
    //替换成mock数据
    const [data, setData] = useState([])
    //书籍名称搜索框的分类下拉
    const [bookList, setBookList] = useState<BookType[]>([]);
    const [userList, setUserList] = useState<UserType[]>([]);

    const [borrowList, setBorrowList] = useState<BorrowType[]>([]);

    //刷新请求数据
    async function fetchData(search?: BorrowQueryType)
    {
        const res = await getBorrowList({
            current: pagination.current,
            pageSize: pagination.pageSize,
            ...search, //搜索栏框里的数据
        })
        console.log("后端返回原始数据:", res.data);
        const newData = res.data.map((item: any) =>({
            ...item,
            book: item.book,
            borrowUser: item.borrowUser,
        }))
        setData(newData)
        setPagination({...pagination, total:res.total})
    }
    // useEffect是当浏览器页面渲染结束后执行
    useEffect( () => {
        fetchData();
        getBookList({all:true}).then((res)=>{
            setBookList(res.data);
        });
        getUserList().then((res)=>{
            setUserList(res.data);
        });
        getBorrowList().then((res)=>{
            setBorrowList(res.data);
        })
    }, []);

    //点击搜索时应该要拿到表单里的值
    const handleSearchFinish = async (values: BorrowQueryType) => {
        // console.log(values)
        const res = await getBorrowList({...values, current:1, pageSize:pagination.pageSize})
        const newData = res.data.map((item: BorrowType) =>({
            ...item,
            book: item.book,
            borrowUser: item.borrowUser.nickName,
        }))
        setData(newData)
        setPagination({...pagination, current: 1,pageSize: pagination.pageSize})
    }
    // 搜索框重置
    const handleSearchReset =() => {
        form.resetFields()
    }
    // 表单每行的编辑按钮跳转界面
    const handleBorrowEdit =(id: string) => {
        router.push(`/borrow/edit/${id}`)
    }
    // 分页情况改变时触发
    const handleTableChange=(pagination: TablePaginationConfig) => {
        setPagination(pagination)
        const query = form.getFieldsValue()
        getBorrowList({
            current: pagination.current,
            pageSize: pagination.pageSize,
            ...query
        })
    }

    // 表单最后增加一列删除、编辑的操作
    const columns = [...COLUMNS,
    {
        title:'操作',key:"action",render:( _: any, row : any) => {
            console.log("row.status =", row.status);
            return <>
                {row.status === "on" ? (<Button type="link" onClick={() => {
                    handleBorrowEdit(row._id)
                }}>归还</Button>): null}
                <Popconfirm
                    title="确认删除？"
                    onConfirm={async () => {
                        await borrowDelete(row._id);
                        message.success("删除成功");
                        alert("删除成功");               // 弹出浏览器提示
                        fetchData(form.getFieldsValue());
                    }}
                    okText="确定"
                    cancelText="取消"
                    okButtonProps={{ danger: true, type: 'primary', size: 'middle' }}
                    cancelButtonProps={{ size: 'middle' }}
                    placement="topRight"
                >
                    <Button type="link" danger>
                        删除
                    </Button>
                </Popconfirm>
            </>
        }
    }
    ]
    return (
    <Content
        title="借阅列表"
        operation={
            <Button type="primary" onClick={ ()=> {
                router.push("/borrow/add");
            }}>
                添加
            </Button>
        }
    >
        <Form
          className={styles.formcss}
          name="search"
          layout="inline"
          form={form}
          onFinish={handleSearchFinish}
          initialValues={{
            book:'', status:'', borrowUser:'',
          }}
        >
        {/* 为了伸缩自适应 */}
        <Row gutter={24}>
            <Col span={7}>
              <Form.Item name="book" label="书籍名称" >
                <Select
                    placeholder="请选择"
                    allowClear
                    showSearch
                    optionFilterProp="label"
                    style={{width: 200}}
                    options={bookList.map(item => ({label: item.name, value: item._id}))}/>
              </Form.Item>
            </Col>
            <Col span={7}>
              <Form.Item name="status" label="状态" >
                <Select
                    placeholder="请选择"
                    allowClear
                    showSearch
                    style={{width: 200}}
                    options={STATUS_OPTIONS}/>
              </Form.Item>
            </Col>
            <Col span={7}>
              <Form.Item name="borrowUser" label="借阅人" >
                <Select
                    placeholder="请选择"
                    allowClear
                    showSearch
                    style={{width: 200}}
                    options={userList.map((item) => ({
                        label: item.nickName,
                        value: item._id,
                    }))}/>
              </Form.Item>
            </Col>
            <Col span={3}>
                <Form.Item>
                    <Space>
                        <Button type="primary" htmlType="submit">
                          搜索
                        </Button>
                          <Button htmlType="submit" onClick={handleSearchReset}>
                          清空
                        </Button>
                    </Space>
                </Form.Item>
            </Col>
        </Row>
        </Form>

        <div className={styles.tablewrap}>
        <Table
            dataSource={data}
            columns={columns}
            scroll={{ y: 500 }}  // 表格主体固定高度，超过就出现滚动条
            rowKey="_id"
            onChange={handleTableChange}
            pagination={{...pagination, showTotal:() => `共 ${pagination.total} 条`  }}
        />
        </div>

    </Content>
    )
}
